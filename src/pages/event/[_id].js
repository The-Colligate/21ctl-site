...
export async function getStaticPaths() {
    ...
    const pages = (await import(`../../../data/eventsData`)).default;
  
    const id = eventsArray.map((page) => page._id);
  
    const paths = id.map((_id) => ({ params: { _id } }));
  ...
  export async function getStaticProps({ params: { _id } }) {
    ...
    const page = eventsArray.find((x) => x._id === _id);
